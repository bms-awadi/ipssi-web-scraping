    ...
    def process_item(self, item, spider):
        ...
        try:
            self.cx.execute(
                ...
                "(entreprise,titre,url,source,date_publi,resume,score_alerte) "
                "VALUES(?,?,?,?,?,?,?)",
                (
                    a.get("entreprise", ""), a["titre"], a.get("url", ""),
                    a.get("source", ""), a.get("date_publi", ""),
                    a.get("resume", ""), a.get("score_alerte", 0),
                ),
            )
            self.cx.commit()
